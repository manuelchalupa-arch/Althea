...
export type RoutineData = {
  ...
  rotationDays: number
  cycle: CycleConfig
  dayExercises: Record<number, { id:string; exId:string; sets:number; reps:number; weight:number; gifUrl?:string; name?:string; muscle?:string; restSec?:number; seriesType?:string }[]>
}
...
export async function getActiveRoutine(): Promise<RoutineData | null> {
  ...
  return (r && (r as any).dayExercises) ? (r as unknown as RoutineData) : null
}
...
export async function saveAllRoutines(list: RoutineData[], activeId: string | null): Promise<void> {
  await db.routineStore.bulkPut(list as unknown as any[])
  if (activeId) {
    await db.routineStore.put({ id: ACTIVE_ID_KEY, activeId, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() } as any)
  }
}

export async function saveRoutine(routine: RoutineData): Promise<void> {
  await db.routineStore.put(routine as unknown as any)
}

export async function setActiveRoutineId(id: string): Promise<void> {
  await db.routineStore.put({ id: ACTIVE_ID_KEY, activeId: id, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() } as any)
}
