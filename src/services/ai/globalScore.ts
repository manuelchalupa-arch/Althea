...
export async function buildGlobalScore(): Promise<GlobalScore> {
  ...
  try {
    const arr: HydrationLog[] = await db.hydrationLogs.where('localDate').equals(today).toArray().catch((): HydrationLog[] => [])
  ...
  try {
    await db.coachMemory.put({ id: `score-${today}`, type: 'score', date: today, score: result.score, factors: result.factors, createdAt: new Date().toISOString() } as CoachMemoryEntry)
  } catch { /* noop */ }
  return result
}
...
export async function getScoreHistory(limit = 30): Promise<Array<{ date: string; score: number }>> {
  try {
    const rows = await db.coachMemory.where('type').equals('score').toArray().catch(() => [])
    return rows
      .map((r) => ({ date: String(r.date || ''), score: Number((r as { score?: number }).score ?? 0) }))
